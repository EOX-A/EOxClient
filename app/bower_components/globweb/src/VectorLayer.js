 ...
 */
	var VectorLayer = function(options) {
		...
		else this.style = new FeatureStyle();
...
*/
	...
	/** @export
	 *  Enable effect animation
	 */
	VectorLayer.prototype.animate = function(start, end) // in seconds
	{
		if (!this.animateableRenderer) {
			var factories = this.globe.vectorRendererManager.factories;
			for (var i = 0; i < factories.length; i++) {
				var factory = factories[i];
				// FIXXME: hack to find the EffectRenderer (typed classes would be great :-))
				var style = {
					"renderer": "pulsar"
				};
				var type = "Point";
		...
		// FIXXME: use requestAnimationFrame!
		var frametick = function() {
			setTimeout(function() {
				//console.debug("[VectorLayer::frametick]: idx = " + idx);
				
				idx += 0.016;
				self.animatedRenderer.instance.update(idx);

				if (idx <= end) {
					frametick();
				} else {
					//console.log("ended animation at " + (idx - 0.016));
				}
			}, 16);
		};
		frametick();
